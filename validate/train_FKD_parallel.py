...
sys.path.append('../')
from relabel.utils_fkd import ImageFolder_FKD_MIX_with_db, ComposeWithCoords, RandomResizedCropWithCoords, \
...
def make_vit_student(num_classes: int = 1000):
    """
    Build a ViT student model with configurable num_classes.
    Uses timm ViT-S, no pretraining.
    """
    import timm
...
def get_args():
    ...
    parser.add_argument("--min-scale-crops", type=float, default=0.25,
    ...
    parser.add_argument('--wandb-api-key', type=str,
                        default='85c395bebc4ca7cc3ebf4e6fdacac9b0f54de5e3', help='wandb api key')
    ...
    parser.add_argument('--dataset', type=str, default='imagenet',
                        ...
                        help='dataset name (controls num_classes and val loader)')
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    train_dataset = ImageFolder_FKD_MIX_with_db(
    ...
    if args.dataset == 'imagenet':
        ...
        # CIFAR-100 test split; root is args.val_dir (e.g. recover/data/cifar-100-python)
        ...
        val_dataset = datasets.CIFAR100(
            ...
            transform=transforms.Compose([
                transforms.Resize(256),        # upsample to match ImageNet-style models
                transforms.CenterCrop(224),
                transforms.ToTensor(),
                normalize,
            ])
    ...
    if args.model in torchvision.models.__dict__:
        ...
        # Adjust classifier head for CIFAR-100 if needed
        if num_classes != 1000:
            if hasattr(model, "fc") and isinstance(model.fc, nn.Linear):
                in_features = model.fc.in_features
                model.fc = nn.Linear(in_features, num_classes)
            elif hasattr(model, "classifier"):  # e.g., MobileNet / EfficientNet style
                # handle the common "classifier" cases if you ever use them
                try:
                    # many torchvision models use a Sequential with last Linear
                    last = model.classifier[-1]
                    if isinstance(last, nn.Linear):
                        in_features = last.in_features
                        model.classifier[-1] = nn.Linear(in_features, num_classes)
                except Exception:
                    pass  # fallback: keep as-is if not needed
    elif args.model == "vit_s_16":
        model = make_vit_student(num_classes=num_classes)
    else:
    ...
    if args.cos == True:
        scheduler = LambdaLR(optimizer,
                             lambda step: 0.5 * (
                                     1. + math.cos(math.pi * step / (2.5*args.epochs))) if step <= (args.epochs*5/6) else 0.5 * (
                                     1. + math.cos(math.pi * 5 / (6 * 2.5))) * (6*args.epochs-6*step)/(6*args.epochs),
                             last_epoch=-1)
    else:
        scheduler = LambdaLR(optimizer,
...
def train(model, args, epoch=None, gpu=0, ngpus_per_node=1, scaler=None):
    ...
    for batch_idx, batch_data in enumerate(args.train_loader):
        ...
        for accum_id in range(0, accum_step * ngpus_per_node, ngpus_per_node):
            ...
            elif args.loss_type == "mae_gt":
                loss = F.l1_loss(output, partial_soft_label) + F.cross_entropy(output, partial_target) * args.ce_weight
            else:
