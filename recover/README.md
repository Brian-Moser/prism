...
To recover knowledge from a pre-trained model and synthesize distilled data, we provide scripts for different ResNet architectures:
- `recover_r18.sh` - ResNet-18
- `recover_r50.sh` - ResNet-50
- `recover_r101.sh` - ResNet-101

Example usage with ResNet-18:
```bash
python data_synthesis.py \
    --arch-name "resnet18" \
    --exp-name "full_r18_10" \
