...
from utils_fkd import ImageFolder_FKD_MIX_with_db, ComposeWithCoords, RandomResizedCropWithCoords, RandomHorizontalFlipWithRes, \
    mix_aug
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    aux_teacher = ["resnet101", "mobilenet_v2", "alexnet", "shufflenet_v2_x0_5"][:args.candidate_number]  # "densenet121
...
def save(train_loader, model, dir_path, args):
    ...
    for batch_idx, (images, target, flip_status, coords_status, index) in enumerate(train_loader):
        ...
        if len(total_output) > 1:
            weighted_output = total_output[0] * 0.5
            remaining_weight = 1.0 - 0.5
            if len(total_output) > 1: # Avoid division by zero if only one model
                weight_per_other_model = remaining_weight / (len(total_output) - 1)
                for i in range(1, len(total_output)):
                    weighted_output += total_output[i] * weight_per_other_model
            output = weighted_output
        else: # Only one model, so its output is the final output
